...
object EnterpriseSettings {
  ...
  private def httpEnterpriseClient(config: Configuration) = Def.task {
    ...
    if (settingApiToken.isEmpty) {
      throw new IllegalStateException("Gatling / apiToken has not been specified. See https://gatling.io/docs/enterprise/cloud/reference/admin/api_tokens to create one.")
    }
  ...
  private def uploadEnterprisePackage(config: Configuration) = Def.task {
    ...
    if (settingPackageId.isEmpty) {
      throw new IllegalStateException("Gatling / packageId has not been specified. See https://gatling.io/docs/enterprise/cloud/reference/user/package_conf/ to create one.")
    }
  ...
  private def startEnterpriseSimulation(config: Configuration) = Def.task {
    ...
    val simulationAndRunSummary = if (settingSimulationId.isEmpty) {
      ...
      if (defaultSimulationClassname.isEmpty) {
        throw new IllegalStateException("Gatling / enterpriseDefaultSimulationClassname has not been specified. See https://gatling.io/docs/gatling/reference/current/extensions/sbt_plugin/.")
      }
    ...
    streams.value.log.success(s"Simulation successfully started; once running, report will be available at ${baseUrl.toExternalForm + simulationAndRunSummary.runSummary.reportsPath}")
  }
